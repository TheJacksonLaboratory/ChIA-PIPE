...
void init_AlignmentTag(tag_opt_t *o)
{
	initTag(o);
	...
	init_ssw_Tag(&(o->align_hlA), &(o->halfLinkerA), LINKER_A);
	init_ssw_Tag(&(o->align_hlB), &(o->halfLinkerB), LINKER_B);
	init_ssw_Tag(&(o->align_hlrcA), &(o->rc_halfLinkerA), LINKER_RC_A);
	init_ssw_Tag(&(o->align_hlrcB), &(o->rc_halfLinkerB), LINKER_RC_B);
	
	init_ssw_Tag(&(o->align_flAA), &(o->fullLinkerAA), LINKER_AA);
	init_ssw_Tag(&(o->align_flBB), &(o->fullLinkerBB), LINKER_BB);
	init_ssw_Tag(&(o->align_flAB), &(o->fullLinkerAB), LINKER_AB);
	init_ssw_Tag(&(o->align_flBA), &(o->fullLinkerBA), LINKER_BA);
}

void initTagAlignmentParameters(tag_opt_t *o)
{
...
tag_opt_t *tag_opt_init()
{
	...
	ksprintf(&(o->halfLinkerA), "%s", G_CP_HalfLinkerA);
	ksprintf(&(o->halfLinkerB), "%s", G_CP_HalfLinkerB);
	...
	memset(&(o->align_hlB), 0, sizeof(tag_t));
	memset(&(o->align_hlrcA), 0, sizeof(tag_t));
	memset(&(o->align_hlrcB), 0, sizeof(tag_t));

	memset(&(o->align_flAA), 0, sizeof(tag_t));
	memset(&(o->align_flBB), 0, sizeof(tag_t));
	memset(&(o->align_flAB), 0, sizeof(tag_t));
	memset(&(o->align_flBA), 0, sizeof(tag_t));
...
void tag_opt_terminate(tag_opt_t *o)
{
	free(o->adapter5.s);
	free(o->adapter3.s);
	...
	free(o->align_hlB.tag_num);
	free(o->align_hlrcA.tag_num);
	free(o->align_hlrcB.tag_num);
	
	free(o->align_flAA.tag_num);
	free(o->align_flBB.tag_num);
	free(o->align_flAB.tag_num);
	free(o->align_flBA.tag_num);
...
static inline int flOverlap5prime(const s_align *pa, const tag_opt_t *o, const bseq1_t *seqs)
{
...
static inline int flOverlap3prime(const s_align *pa, const tag_opt_t *o, const bseq1_t *seqs)
{
	if (pa->score1<o->filterFLOverlap) return 0; // did not reach the minimum score
...
static inline int hlOverlap5prime(const s_align *pa, const tag_opt_t *o, const bseq1_t *seqs)
{
...
static inline int hlOverlap3prime(const s_align *pa, const tag_opt_t *o, const bseq1_t *seqs)
{
	if (pa->score1<o->filterHLOverlap) return 0; // did not reach the minimum score
...
static void tag_worker(void *data, int i, int tid)
{
	worker_t *w = (worker_t*)data;
	if (!(w->opt->flag&MEM_F_PE)) {
		// single-end sequence
		// adjust the read length if in effective
		if (w->opt->maxReadLength>0) {
			if (w->seqs[i].l_seq>w->opt->maxReadLength) {
				w->seqs[i].l_seq = w->opt->maxReadLength;
				w->seqs[i].seq[w->opt->maxReadLength] = '\0';
				w->seqs[i].qual[w->opt->maxReadLength] = '\0';
			}
		}
		...
		if (resNTrc3p->score1>=w->opt->filterAdapter3) {
			...
			if (0!=nexteraRCAdapter3Overlap3prime(aRec.paa, w->opt, &w->seqs[i])) {
		...
		s_align *resflAB1 = 0; const tag_t *flAB1 = &(w->opt->align_flAB);
		resflAB1 = ssw_align (p1, flAB1->tag_num, flAB1->tag_bufsize, w->opt->linker_gap_open, w->opt->linker_gap_extension, w->opt->linker_aln_flag, w->opt->linker_filter, 0, maskLen);
		
		if (resflAA1->score1 > resflAB1->score1) {
			aRec.linker_type = LINKER_AA; aRec.pla = resflAA1;
			align_destroy(resflAB1);
		} else if (resflAA1->score1 < resflAB1->score1) {
			aRec.linker_type = LINKER_AB; aRec.pla = resflAB1;
			align_destroy(resflAA1);
		} else {
			// a tie, does it matter?
			aRec.linker_type = LINKER_AA; aRec.pla = resflAA1; aRec.tied_linker_type = LINKER_AB;
			align_destroy(resflAB1);
		}
		
		s_align *resflBB1 = 0; const tag_t *flBB1 = &(w->opt->align_flBB);
		resflBB1 = ssw_align (p1, flBB1->tag_num, flBB1->tag_bufsize, w->opt->linker_gap_open, w->opt->linker_gap_extension, w->opt->linker_aln_flag, w->opt->linker_filter, 0, maskLen);
		if (resflBB1->score1 > aRec.pla->score1) {
			align_destroy(aRec.pla);
			aRec.linker_type = LINKER_BB; aRec.pla = resflBB1; aRec.tied_linker_type = LINKER_NONE;
		} else if (resflBB1->score1 < aRec.pla->score1) {
			// do nothing
			align_destroy(resflBB1);
		} else {
			// a tie, does it matter?
			aRec.tied_linker_type = LINKER_BB;
			align_destroy(resflBB1);
		}
		
		s_align *resflBA1 = 0; const tag_t *flBA1 = &(w->opt->align_flBA);
		resflBA1 = ssw_align (p1, flBA1->tag_num, flBA1->tag_bufsize, w->opt->linker_gap_open, w->opt->linker_gap_extension, w->opt->linker_aln_flag, w->opt->linker_filter, 0, maskLen);
		if (resflBA1->score1 > aRec.pla->score1) {
			align_destroy(aRec.pla);
			aRec.linker_type = LINKER_BA; aRec.pla = resflBA1; aRec.tied_linker_type = LINKER_NONE;
		} else if (resflBA1->score1 < aRec.pla->score1) {
			// do nothing
			align_destroy(resflBA1);
		} else {
			// a tie, does it matter?
			aRec.tied_linker_type = LINKER_BA;
			align_destroy(resflBA1);
		}
		...
		if (aRec.pla->score1>=w->opt->filterFL) {
			...
			if (0!=flOverlap5prime(aRec.pla, w->opt, &w->seqs[i])) {
				// okie, we have a prefix fix case
			} else if (0!=flOverlap3prime(aRec.pla, w->opt, &w->seqs[i])) {
				// okie, we have a suffix fix case
			} else {
				
				// try half linker solution
				align_destroy(aRec.pla);
				aRec.linker_type = LINKER_NONE; aRec.pla = 0; aRec.tied_linker_type = LINKER_NONE;
				...
				s_align *reshlB1 = 0; const tag_t *hlB1 = &(w->opt->align_hlB);
				reshlB1 = ssw_align (p1, hlB1->tag_num, hlB1->tag_bufsize, w->opt->linker_gap_open, w->opt->linker_gap_extension, w->opt->linker_aln_flag, w->opt->linker_filter, 0, maskLen);
				
				if (reshlA1->score1 > reshlB1->score1) {
					aRec.linker_type = LINKER_A; aRec.pla = reshlA1;
					align_destroy(reshlB1);
				} else if (reshlA1->score1 < reshlB1->score1) {
					aRec.linker_type = LINKER_B; aRec.pla = reshlB1;
					align_destroy(reshlA1);
				} else {
					// a tie, does it matter?
					aRec.linker_type = LINKER_A; aRec.pla = reshlA1; aRec.tied_linker_type = LINKER_B;
					align_destroy(reshlB1);
				}
				...
				if (reshlA1rc->score1 > aRec.pla->score1) {
					align_destroy(aRec.pla);
					aRec.linker_type = LINKER_RC_A; aRec.pla = reshlA1rc; aRec.tied_linker_type = LINKER_NONE;
				} else if (reshlA1rc->score1 < aRec.pla->score1) {
					// do nothing
					align_destroy(reshlA1rc);
				} else {
					// a tie, does it matter?
					aRec.tied_linker_type = LINKER_RC_A;
					align_destroy(reshlA1rc);
				}
				
				s_align *reshlB1rc = 0; const tag_t *hlB1rc = &(w->opt->align_hlrcB);
				reshlB1rc = ssw_align (p1, hlB1rc->tag_num, hlB1rc->tag_bufsize, w->opt->linker_gap_open, w->opt->linker_gap_extension, w->opt->linker_aln_flag, w->opt->linker_filter, 0, maskLen);
				
				if (reshlB1rc->score1 > aRec.pla->score1) {
					align_destroy(aRec.pla);
					aRec.linker_type = LINKER_RC_B; aRec.pla = reshlB1rc;aRec.tied_linker_type = LINKER_NONE;
				} else if (reshlB1rc->score1 < aRec.pla->score1) {
					// do nothing
					align_destroy(reshlB1rc);
				} else {
					// a tie, does it matter?
					aRec.tied_linker_type = LINKER_RC_B;
					align_destroy(reshlB1rc);
				}
				...
				if (aRec.pla->score1>=w->opt->filterHL) {
					...
					if (0!=hlOverlap5prime(aRec.pla, w->opt, &w->seqs[i])) {
						// okie, we have a prefix fix case
					} else if (0!=hlOverlap3prime(aRec.pla, w->opt, &w->seqs[i])) {
						// okie, we have a suffix fix case
					} else {
		...
		processSingleTag (i + w->n_processed + 1,
						  &(w->cpus[i]), &(w->seqs[i]), &(w->cpu_fqs[i]),
						  w->opt->minTagLen, w->opt->cpuflag & (CPU_FASTQ|CPU_RAW|CPU_RC_READ), CPU_TAGTYPE_DILINKER);
		
	} else {
		// adjust the read length if in effective
		if (w->opt->maxReadLength>0) {
			if (w->seqs[i<<1|0].l_seq>w->opt->maxReadLength) {
				w->seqs[i<<1|0].l_seq = w->opt->maxReadLength;
				w->seqs[i<<1|0].seq[w->opt->maxReadLength] = '\0';
				w->seqs[i<<1|0].qual[w->opt->maxReadLength] = '\0';
			}
			if (w->seqs[i<<1|1].l_seq>w->opt->maxReadLength) {
				w->seqs[i<<1|1].l_seq = w->opt->maxReadLength;
				w->seqs[i<<1|1].seq[w->opt->maxReadLength] = '\0';
				w->seqs[i<<1|1].qual[w->opt->maxReadLength] = '\0';
			}
		}
		...
		s_align *resflAB2 = 0; const tag_t *flAB2 = &(w->opt->align_flAB);
		resflAB2 = ssw_align (p2, flAB2->tag_num, flAB2->tag_bufsize, w->opt->linker_gap_open, w->opt->linker_gap_extension, w->opt->linker_aln_flag, w->opt->linker_filter, 0, maskLen);
		
		if (resflAA2->score1 > resflAB2->score1) {
			aRec.linker_type = LINKER_AA; aRec.pla = resflAA2;
			align_destroy(resflAB2);
		} else if (resflAA2->score1 < resflAB2->score1) {
			aRec.linker_type = LINKER_AB; aRec.pla = resflAB2;
			align_destroy(resflAA2);
		} else {
			// a tie, does it matter?
			aRec.linker_type = LINKER_AA; aRec.pla = resflAA2; aRec.tied_linker_type = LINKER_AB;
			align_destroy(resflAB2);
		}
		
		s_align *resflBB2 = 0; const tag_t *flBB2 = &(w->opt->align_flBB);
		resflBB2 = ssw_align (p2, flBB2->tag_num, flBB2->tag_bufsize, w->opt->linker_gap_open, w->opt->linker_gap_extension, w->opt->linker_aln_flag, w->opt->linker_filter, 0, maskLen);
		if (resflBB2->score1 > aRec.pla->score1) {
			align_destroy(aRec.pla);
			aRec.linker_type = LINKER_BB; aRec.pla = resflBB2; aRec.tied_linker_type = LINKER_NONE;
		} else if (resflBB2->score1 < aRec.pla->score1) {
			// do nothing
			align_destroy(resflBB2);
		} else {
			// a tie, does it matter?
			aRec.tied_linker_type = LINKER_BB;
			align_destroy(resflBB2);
		...
		s_align *resflBA2 = 0; const tag_t *flBA2 = &(w->opt->align_flBA);
		resflBA2 = ssw_align (p2, flBA2->tag_num, flBA2->tag_bufsize, w->opt->linker_gap_open, w->opt->linker_gap_extension, w->opt->linker_aln_flag, w->opt->linker_filter, 0, maskLen);
		if (resflBA2->score1 > aRec.pla->score1) {
			align_destroy(aRec.pla);
			aRec.linker_type = LINKER_BA; aRec.pla = resflBA2; aRec.tied_linker_type = LINKER_NONE;
		} else if (resflBA2->score1 < aRec.pla->score1) {
			// do nothing
			align_destroy(resflBA2);
		} else {
			// a tie, does it matter?
			aRec.tied_linker_type = LINKER_BA;
			align_destroy(resflBA2);
		...
		if (aRec.pla->score1>=w->opt->filterFL) {
			...
			if (0!=flOverlap5prime(aRec.pla, w->opt, &w->seqs[i<<1|1])) {
				...
				s_align *reshlB2 = 0; const tag_t *hlB2 = &(w->opt->align_hlB);
				reshlB2 = ssw_align (p2, hlB2->tag_num, hlB2->tag_bufsize, w->opt->linker_gap_open, w->opt->linker_gap_extension, w->opt->linker_aln_flag, w->opt->linker_filter, 0, maskLen);
				
				if (reshlA2->score1 > reshlB2->score1) {
					aRec.linker_type = LINKER_A; aRec.pla = reshlA2;
					align_destroy(reshlB2);
				} else if (reshlA2->score1 < reshlB2->score1) {
					aRec.linker_type = LINKER_B; aRec.pla = reshlB2;
					align_destroy(reshlA2);
				} else {
					// a tie, does it matter?
					aRec.linker_type = LINKER_A; aRec.pla = reshlA2; aRec.tied_linker_type = LINKER_B;
					align_destroy(reshlB2);
				}
				...
				if (reshlA2rc->score1 > aRec.pla->score1) {
					align_destroy(aRec.pla);
					aRec.linker_type = LINKER_RC_A; aRec.pla = reshlA2rc; aRec.tied_linker_type = LINKER_NONE;
				} else if (reshlA2rc->score1 < aRec.pla->score1) {
					// do nothing
				...
				s_align *reshlB2rc = 0; const tag_t *hlB2rc = &(w->opt->align_hlrcB);
				reshlB2rc = ssw_align (p2, hlB2rc->tag_num, hlB2rc->tag_bufsize, w->opt->linker_gap_open, w->opt->linker_gap_extension, w->opt->linker_aln_flag, w->opt->linker_filter, 0, maskLen);
				
				if (reshlB2rc->score1 > aRec.pla->score1) {
					align_destroy(aRec.pla);
					aRec.linker_type = LINKER_RC_B; aRec.pla = reshlB2rc; aRec.tied_linker_type = LINKER_NONE;
				} else if (reshlB2rc->score1 < aRec.pla->score1) {
					// do nothing
					align_destroy(reshlB2rc);
				} else {
					// a tie, does it matter?
					aRec.tied_linker_type = LINKER_RC_B;
					align_destroy(reshlB2rc);
		...
		// TODO: check that reverse complement is set up for calling
		// perform the tags
...
void tag_process_seqs(const tag_opt_t *opt, bseq1_t *seqs, cpu_record_t *cpus, cpu_fq_t *cpu_fqs, int64_t n_processed, int n)
{
	...
	kt_for(opt->n_threads, tag_worker, &w, (opt->flag&MEM_F_PE)? n>>1 : n); // find mapping positions
}
...
int main_tag(int argc, char *argv[])
{
	...
	opt = tag_opt_init();
	while ((c = getopt(argc, argv, "RFWLNCSQp5:3:A:B:t:T:O:M:")) >= 0) {
		if (c == 't') opt->n_threads = atoi(optarg), opt->n_threads = opt->n_threads > 1? opt->n_threads : 1;
			...
			if (isdigit(optarg[0])) {
				long value = strtol(optarg, &p, 10);
				if (value>0) opt->filterAdapter5 = (int32_t) value;
				if (*p != 0 && ispunct(*p) && ('A'==p[1] || 'C'==p[1] || 'G'==p[1] || 'T'==p[1]))
					ks_set(p+1, &(opt->adapter5));
			} else {
				ks_set(optarg, &(opt->adapter5));
			}
		}
		else if (c == '3') {
			if (isdigit(optarg[0])) {
				long value = strtol(optarg, &p, 10);
				if (value>0) opt->filterAdapter3 = (int32_t) value;
				if (*p != 0 && ispunct(*p) && ('A'==p[1] || 'C'==p[1] || 'G'==p[1] || 'T'==p[1]))
					ks_set(p+1, &(opt->adapter3));
			} else {
				ks_set(optarg, &(opt->adapter3));
			}
		}
		...
		else if (c == 'M') opt->maxReadLength = atoi(optarg), opt->minTagLen = opt->minTagLen > 0 ? opt->minTagLen : -1;
		else {
			tag_opt_terminate(opt);
			free(opt);
			return 1;
		}
	}
	...
	initTag(opt); // TODO: is this necessary?
	initTagAlignmentParameters(opt);
		...
		if (0==(opt->cpuflag & CPU_DEBUG_MASK)) {
			...
			if (opt->flag & MEM_F_PE) {
				...
				for (i = 0; i < n; i++) {
					...
					gzprintf(outfd, "\t%d\t%d", pCPU1->len[0], pCPU1->len[1]);
					
					gzprintf(outfd, "\t%s", seqs[i].name);
					
					if (opt->cpuflag & CPU_SEQ) {
						gzprintf(outfd, "\t%s", seqs[i].seq);
					}
					if (opt->cpuflag & CPU_QUALITY) {
						gzprintf(outfd, "\t%s", seqs[i].qual);
					}
