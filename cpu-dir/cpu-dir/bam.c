...
// WCH
int bam_read1_raw(bamFile fp, bam1_t *b, uint32_t x[8])
{
	// just read in the raw data
	...
	if (bam_is_be) bam_swap_endian_4p(&block_len);
...
int bam_read1_parse(bam1_t *b, uint32_t x[8])
{
	int32_t i;
	bam1_core_t *c = &b->core;
	
	// will parse the data
	if (bam_is_be) for (i = 0; i < 8; ++i) bam_swap_endian_4p(x + i);
	...
	return 4 + b->data_len + BAM_CORE_SIZE;
}
// END-WCH
