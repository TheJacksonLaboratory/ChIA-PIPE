...
// OBJECTIVE: testing multi-threading version
//            the multi-threading works, but the gain is only in the sorting but the bulk of the time is in writing out bam file
...
mtdedup_opt_t *mtdedup_opt_init()
{
	mtdedup_opt_t *o;
	o = calloc(1, sizeof(mtdedup_opt_t));
...
void mtdedup_opt_terminate(mtdedup_opt_t *o)
{
	free(o->outputPrefix.s);
}

typedef struct {
	uint32_t n1DPartitions;
	uint32_t n2DPartitions;
	uint32_t *n2DPartitionSizes;
} mate_partitions_t;

void initMatePartition (mate_partitions_t *mp, uint32_t nTargets) {
	mp->n1DPartitions = (nTargets+1);
	mp->n2DPartitions = mp->n1DPartitions * mp->n1DPartitions;
	mp->n2DPartitionSizes = calloc(mp->n2DPartitions, sizeof(uint32_t));
}

int32_t getMatePartitionIndex (const mate_partitions_t *mp, int32_t tid, int32_t mtid) {
	return ((-1==tid?0:tid+1) * mp->n1DPartitions + (-1==mtid?0:mtid+1));
}

void terminateMatePartition (mate_partitions_t *mp) {
	free(mp->n2DPartitionSizes);
}

typedef struct {
	mate_partitions_t partition;
	cpu_dedup_record_t **dedupRecords;
} worker_dedup_sort_2nd_pass_t;

typedef struct {
	const mtdedup_opt_t *opt;
...
static void dedup_worker(void *data, int i, int tid)
{
	...
	// ENHANCEMENT:
	if (w->mps) {
		w->mps[tid].n2DPartitionSizes[getMatePartitionIndex(&(w->mps[tid]), dedupRec->tid, dedupRec->mtid)]++;
	}
...
void mtdedup_process_alns(const mtdedup_opt_t *opt, uint32_t n_processed, int n, bam1_t *bamRecs, cpu_readid_t *readids, uint32_t *pNumTagGroups, tag_group_t *tagGroups, cpu_dedup_record_t *dedupRecords, mate_partitions_t *mp, CPUBamBuffer_t* bamsBuffer, uint32_t nUpdateBam)
{
	int i,j;
	...
	if (mp) {
		w.mps = calloc(opt->n_threads, sizeof(mate_partitions_t));
		for(i=0; i<opt->n_threads; ++i) {
			initMatePartition(&(w.mps[i]), mp->n1DPartitions-1);
		}
	} else {
		w.mps = NULL;
	}
	...
	*pNumTagGroups = numTagGroups;
	...
	if (mp) {
		for(i=0; i<opt->n_threads; ++i) {
			for(j=0; j<mp->n2DPartitions; ++j) {
				//wSort2ndPass.nPartitionSizes[(-1==dedupRecords[i].tid)?0:dedupRecords[i].tid+1]++;
				mp->n2DPartitionSizes[j] += w.mps[i].n2DPartitionSizes[j];
			}
			terminateMatePartition(&(w.mps[i]));
		}
	}
}

int mtupdateDedupTotalRecords(const char *dedupKeyFilename, uint32_t n_pairProcessed) {
	const char *mode = "r+b";
...
// TODO:
cpu_dedup_record_t *mtreadDedupKeys(const char *dedupKeyFilename, uint32_t *nDedupKeys) {
	*nDedupKeys = 0;
...
// first compare only the chromosomes only
// then, by the rest of the fields
int cmpPETFirstPass(const void *a, const void *b) {
...
int cmpPETSecondPass(const void *a, const void *b) {
	const cpu_dedup_record_t *ia = (const cpu_dedup_record_t *)a;
	const cpu_dedup_record_t *ib = (const cpu_dedup_record_t *)b;

	// sort by chrom1, start1, end1,
	// int32_t i32comp = ia->tid - ib->tid; if (0!=i32comp) return i32comp; // done at First Pass
	int32_t i32comp = ia->pos - ib->pos; if (0!=i32comp) return i32comp;
	i32comp = ia->endpos - ib->endpos; if (0!=i32comp) return i32comp;
	
	// follow by chrom2, start2, end2
	//i32comp = ia->mtid - ib->mtid; if (0!=i32comp) return i32comp; // done at First Pass
...
static void qsort_2nd_pass_worker(void *data, int i, int tid)
{
	worker_dedup_sort_2nd_pass_t *w = (worker_dedup_sort_2nd_pass_t*)data;
	if (w->partition.n2DPartitionSizes[i]>0) {
		qsort(w->dedupRecords[i], w->partition.n2DPartitionSizes[i], sizeof(cpu_dedup_record_t), cmpPETSecondPass);
	}
}

int mtcmpPET(const void *a, const void *b) {
	const cpu_dedup_record_t *ia = (const cpu_dedup_record_t *)a;
	const cpu_dedup_record_t *ib = (const cpu_dedup_record_t *)b;
...
int mtisDuplicate(cpu_dedup_record_t *ia, cpu_dedup_record_t *ib) {
	// sort by chrom1, start1, end1,
...
uint32_t *mtmarkDuplicates(cpu_dedup_record_t *dedupKeys, int nDedupKeys, uint32_t n_processed, uint32_t *pUniquePETs, uint32_t *pUnMappedPETs) {
	int i;
	...
	for (i=1; i<nDedupKeys; ++i) {
		...
		if (-1==dedupKeys[i].tid && -1==dedupKeys[i].mtid) {
			...
			if (mtisDuplicate(pRec, &dedupKeys[i])) {
				dedupKeys[i].duplicated = 1;
...
int main_mtdedup(int argc, char *argv[])
{
	...
	if (argc == optind + 1) { // convert/print the entire file
		...
		worker_dedup_sort_2nd_pass_t wSort2ndPass;
		wSort2ndPass.dedupRecords = NULL;
		initMatePartition(&(wSort2ndPass.partition), in->header->n_targets);
		...
		while ((bamRecs = readCPUBam(&n, nNumRequested, in, &bamsBuffer)) != 0) {
			...
			mtdedup_process_alns(opt, n_processed, n, bamRecs, readids, &numTagGroups, tagGroups, dedupRecords, &(wSort2ndPass.partition), &bamsBuffer, 0);
			t_timepointProcessing = realtime();
		...
		t_timepointProcessing = realtime();
		mtupdateDedupTotalRecords(dedupKeyFilename.s, n_pairProcessed);
		t_timepointIO = realtime();
		...
		t_timepointProcessing = realtime();
		cpu_dedup_record_t *dedupKeys = mtreadDedupKeys(dedupKeyFilename.s, &nDedupKeys);
		t_timepointIO = realtime();
		t_diffProcessing = t_timepointIO - t_timepointProcessing;
		if (bwa_verbose >= 3) {
...
#if 0
		//TODO: for DEBUGGING, check what is the order!!!
		qsort(dedupKeys, nDedupKeys, sizeof(cpu_dedup_record_t), cmpPETFirstPass);
		fprintf(stdout, "\n\n===\tPARTITIONS\n");
		for(i=0; i<wSort2ndPass.nPartitions; ++i) {
			if (0==i) {
				fprintf(stdout, "%d\t*\t%d\n", i, wSort2ndPass.nPartitionSizes[i]);
			} else {
				fprintf(stdout, "%d\t%s\t%d\n", i, in->header->target_name[i-1], wSort2ndPass.nPartitionSizes[i]);
			}
		}
		...
		if (opt->disponoParallel) {
			...
			qsort(dedupKeys, nDedupKeys, sizeof(cpu_dedup_record_t), cmpPETFirstPass);
			// get first pass partitions
			// perform multi-threading on working single-threadedly on each partitiions
			// TODO: proof of concept, single threaded, but to change to multiple threads
			wSort2ndPass.dedupRecords = (cpu_dedup_record_t **) calloc(wSort2ndPass.partition.n2DPartitions, sizeof(cpu_dedup_record_t *));
#if 0
			uint32_t nOffset = 0;
			for(i=0; i<wSort2ndPass.partition.n2DPartitions; ++i) {
				cpu_dedup_record_t *partitionDeduKeys = dedupKeys + nOffset;
				wSort2ndPass.dedupRecords[i] = partitionDeduKeys;
				nOffset += wSort2ndPass.partition.n2DPartitionSizes[i];
			}
#else
			uint32_t nOffset = 0;
			uint32_t nNonZeroParitions = 0;
			for(i=0; i<wSort2ndPass.partition.n2DPartitions; ++i) {
				if (wSort2ndPass.partition.n2DPartitionSizes[i]>0) {
					cpu_dedup_record_t *partitionDeduKeys = dedupKeys + nOffset;
					wSort2ndPass.dedupRecords[nNonZeroParitions] = partitionDeduKeys;
					nOffset += wSort2ndPass.partition.n2DPartitionSizes[i];
					
					wSort2ndPass.partition.n2DPartitionSizes[nNonZeroParitions] = wSort2ndPass.partition.n2DPartitionSizes[i];
					nNonZeroParitions++;
				}
			}
#endif
...
#if 0
			// FOR DEBUGGING: check that the partitions are set up correctly!
			//  tid, tidchr, mtid, mtidchr, size, index, ptr->tid, ptr->mtid
			fprintf(stdout, "\n\n===\tPARTITIONS\n");
			fprintf(stdout, "tid\ttidchr\tmtid\tmtidchr\tsize\tindex\tptr->tid\tptr->mtid\n");
			for(i=0; i<wSort2ndPass.partition.n1DPartitions; ++i) {
				for(j=0; j<wSort2ndPass.partition.n1DPartitions; ++j) {
					int32_t index = getMatePartitionIndex(&(wSort2ndPass.partition), i-1, j-1);
					cpu_dedup_record_t *partitionDedupKey = wSort2ndPass.dedupRecords[index];
					fprintf(stdout, "%d\t%s\t%d\t%s\t%d\t%d\t%d\t%d\n",
							i, (0==i ? "*" : in->header->target_name[i-1]),
							j, (0==j ? "*" : in->header->target_name[j-1]),
							wSort2ndPass.partition.n2DPartitionSizes[index],
							index,
							(0==wSort2ndPass.partition.n2DPartitionSizes[index] ? -2 : partitionDedupKey->tid),
							(0==wSort2ndPass.partition.n2DPartitionSizes[index] ? -2 : partitionDedupKey->mtid));
				}
			}
			fprintf(stdout, "===\tEND - PARTITIONS\n");
#endif
			...
			extern void kt_for(int n_threads, void (*func)(void*,int,int), void *data, int n);
#if 0
			kt_for(opt->n_threads,qsort_2nd_pass_worker,&wSort2ndPass,wSort2ndPass.partition.n2DPartitions);
#else
			if (nNonZeroParitions>0) kt_for(opt->n_threads,qsort_2nd_pass_worker,&wSort2ndPass,nNonZeroParitions);
#endif
			//qsort(partitionDeduKeys, partitions[i], sizeof(cpu_dedup_record_t), cmpPETSecondPass);
			
		} else {
			qsort(dedupKeys, nDedupKeys, sizeof(cpu_dedup_record_t), mtcmpPET);
		}
		
		
		terminateMatePartition(&(wSort2ndPass.partition));
		if (wSort2ndPass.dedupRecords) free(wSort2ndPass.dedupRecords);
		...
		uint32_t *duplicates = mtmarkDuplicates(dedupKeys, nDedupKeys, n_processed, &nUniquePETs, &nUnMappedPETs);
		t_timepointIO = realtime();
		t_diffProcessing = t_timepointIO - t_timepointProcessing;
		...
		if (opt->toGroup) {
			...
			while ((bamRecs = readCPUBam(&n, nNumRequested, in, &bamsBuffer)) != 0) {
				...
				uint32_t numTagGroups = 0;
				mtdedup_process_alns(opt, n_processed, n, bamRecs, readids, &numTagGroups, tagGroups, dedupRecords, NULL, &bamsBuffer, 1);
				t_timepointProcessing = realtime();
				t_diffProcessing = t_timepointProcessing - t_timepointIO;
