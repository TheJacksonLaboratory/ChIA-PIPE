...
typedef struct {
	...
	int nBins;
	long bin_under[NUM_PAIR_ORIENTATIONS];
	long bin_over[NUM_PAIR_ORIENTATIONS];
	long *bins;
} span_histogram_t;
...
static inline void init_Span_Histogram (int lower, int upper, int nBins, span_histogram_t *h)
{
	...
	h->nBins = nBins;
	for(i=0; i<NUM_PAIR_ORIENTATIONS; ++i) { h->bin_under[i] = h->bin_over[i] = 0; }
	h->bins = calloc(NUM_PAIR_ORIENTATIONS*nBins, sizeof(long)); // consider pairing information
	if (!h->bins) {
...
static inline void count_Histogram (span_histogram_t *h, int span, int pairOrientation)
{
	// TODO: consider pairing
	int offset = binPairOffset (h, span, pairOrientation);
	
	if (-1==offset) {
		h->bin_under[pairOrientation]++;
	}
	else if (h->nBins==offset) {
		h->bin_over[pairOrientation]++;
	}
	else {
		h->bins[offset*NUM_PAIR_ORIENTATIONS+pairOrientation]++;
	}
}
...
#define BAM_FREAD2       128
						...
						count_Histogram (&(w->spanHistograms[tid]), bam->core.isize, nPairOrientation);
...
void span_process_alns(const span_opt_t *opt, int64_t n_processed, int n, bam1_t *bamRecs, cpu_readid_t *readids, int *pNumTagGroups, tag_group_t *tagGroups, CPUBamBuffer_t* bamsBuffer, span_histogram_t *tspans, span_histogram_t *spans )
{
	...
	*pNumTagGroups = numTagGroups;
	...
	for(i=0; i<opt->n_threads; ++i) {
		for(j=0; j<NUM_PAIR_ORIENTATIONS*spans->nBins;++j) { // there are 4 possible pairings
			spans->bins[j] += tspans[i].bins[j];
		}
		for(j=0; j<NUM_PAIR_ORIENTATIONS; ++j) {
...
int main_span(int argc, char *argv[])
{
	...
	if (totalInBins>0) {
		...
		for(i=0; i<spans.nBins;++i) {
			...
			if (lower<0) lower = 0;
			lValue = 0; k=i*NUM_PAIR_ORIENTATIONS; for(j=k; j<k+4; j++) { lValue += spans.bins[j]; }
			// TODO: consider pairing
			fprintf(stdout, "%d\t%d\t%d\t%d\t%ld\t%.2g%%\t%ld\t%.2g%%\t%ld\t%.2g%%\t%ld\t%.2g%%\t%ld\t%.2g%%\n", i, lower, upper, (int)((lower+upper)/2),
					lValue, lValue*100.0/nTotal,
