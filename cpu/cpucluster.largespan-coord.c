...
// for easier comparison to Guoliang's output
#define DEBUG_USE_MID_POINT	1
...
static void tags_extension_worker(void *data, int i, int tid)
{
	...
	// TODO: optimization; does not need to check tid and mtid as it has to be paired!!!
...
void cluster_count_iPET(const cluster_opt_t *opt, cpu_cluster_record_t *clusterRecords, uint32_t nClusterKeys, uint32_t *numClusterBins) {
	extern void kt_for(int n_threads, void (*func)(void*,int,int), void *data, int n);
	int32_t i;
	//worker_t w;
...
int32_t cluster_bin(const cluster_opt_t *opt, cpu_cluster_record_t *clusterRecords, uint32_t nClusterKeys, int iteration, long pairId) {
	int32_t i,j;
			...
			if (clusterRecords[i].swap) {
				...
				
				{
						...
						if (cid==clusterRecord->cid && mcid==clusterRecord->mcid) {
							...
							if (0==is_overlap(s1, e1, clusterRecord->pos, clusterRecord->endpos)) {
								...
								if (0==is_overlap(s2, e2, clusterRecord->mpos, clusterRecord->mendpos)) {
									// does not overlap! this is a problem, report it
									if (bwa_verbose>=5) {
										fprintf(stderr, "[WARNING:%s:non_overlap_tag2] i=%d, j=%d, iPET=%d, leader=%d, s1=%d, e1=%d, s2=%d(%d), e2=%d(%d), oid=%d(%d), moid=%d(%d)\n",
												__func__,
					...
					if (nBinDiscordantPETs>0) {
						...
						int32_t nRetryDiscordantPETs = cluster_bin(opt, &(clusterRecords[i]), niPET, iteration+1, pairId);
...
#if 1
		const cpu_cluster_record_t *clusterRec = &(clusterKeys[i]);
		//if (0==clusterRec->usable && clusterRec->iPET>0) {
		if (1==clusterRec->usable) {
			if (cid==clusterRec->cid && mcid==clusterRec->mcid) {
				fprintf(stderr, "[E::report_Clusters_List] Still the same cluster after advancement, i=%d, cid=%d, mcid=%d, oid=%d, moid=%d\n",
						i, clusterRec->cid, clusterRec->mcid, clusterRec->oid, clusterRec->moid);
				return;
			}
			...
			if (clusterRec->tid != clusterRec->mtid) {
				...
				// for easier downstream processing, negative distance is represented by positive equivalent
				if (dist<0) dist *= -1;
			}
...
void report_Subclusters_List (FILE *clustersfd, const cpu_subcluster_record_t *subclusters, uint32_t nSubclusters, const bam_header_t *header) {
	...
	fprintf(stderr, "#pairId\toffset\tmerged\tcid\tmcid\toid\tmoid\tiPETs\tpos\tendpos\tmpos\tmendpos\n");
	
	for(i=0; i<nSubclusters; ++i) {
		
		const cpu_subcluster_record_t *subcluster = &(subclusters[i]);
#if 1
		fprintf(stderr, "%lu\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n",
				subcluster->pairId, subcluster->offset, subcluster->merged,
...
int32_t merge_subclusters (const cluster_opt_t *opt, cpu_subcluster_record_t *subclusters, uint32_t nSubclusters, uint32_t nFinalSubclusters) {
	...
	if (nSubclusters>1) {
		...
		// let's sort based on tag right and process them
		if (bwa_verbose >= 5) {
			fprintf(logFH, "[D:SubCluster] initial \n");
		...
		if (bwa_verbose >= 5) {
			fprintf(logFH, "[D:SubCluster] Tag2 ordered \n");
			report_Subclusters_List (logFH, subclusters, nSubclusters, NULL);
		}
		...
		if (bwa_verbose >= 5) {
			fprintf(logFH, "[D:SubCluster] Tag1 ordered \n");
			report_Subclusters_List (logFH, subclusters, nSubclusters, NULL);
		}
		...
		if (bwa_verbose >= 5) {
			fprintf(logFH, "[D:SubCluster] Combined Tag1 and Tag2 ordered, estimated %d subclusters\n", numSubclusters);
			report_Subclusters_List (logFH, subclusters, nSubclusters, NULL);
		}
		...
		// TODO: check condition of merging and update the ranges
		{
			...
			for(j=0; j<nSubclusters; ++j) {
				...
				if (cid==subcluster->cid && mcid==subcluster->mcid) {
					...
					if (0==is_overlap(s1, e1, subcluster->pos, subcluster->endpos)) {
						// does not overlap! this is a problem, report it
						if (bwa_verbose >= 5) {
							fprintf(logFH, "[D:SubCluster:tag1] j=%d, iPET=%d, leader=%d, s1=%d(%d), e1=%d(%d), s2=%d, e2=%d, oid=%d(%d), moid=%d(%d)\n",
						...
						if (0==is_overlap(s2, e2, subcluster->mpos, subcluster->mendpos)) {
							// does not overlap! this is a problem, report it
							if (bwa_verbose >= 5) {
								fprintf(logFH, "[D:SubCluster:tag2] j=%d, iPET=%d, leader=%d, s1=%d, e1=%d, s2=%d(%d), e2=%d(%d), oid=%d(%d), moid=%d(%d)\n",
			...
			if (bwa_verbose >= 5) {
				fprintf(logFH, "[D:SubCluster] After checking correction, with %d merger(s)\n", nMergedSubclusters);
				report_Subclusters_List (logFH, subclusters, nSubclusters, NULL);
			}
			...
			if (nMergedSubclusters>0) {
				...
				if (bwa_verbose >= 5) {
					fprintf(logFH, "[D:SubCluster] Final re-ordering\n");
					report_Subclusters_List (logFH, subclusters, nSubclusters, NULL);
				}
				
				// TODO: check condition of invocation & termination
				//if (numSubclusters!=nSubclusters) {
				int32_t final_numSubclusters = merge_subclusters (opt, subclusters, newNumSubclusters, newNumSubclusters);
				if (bwa_verbose >= 5) {
					fprintf(logFH, "[D:SubCluster] Re-cur merging, triggered by %d, subclusters: %d -> %d \n", nMergedSubclusters, nSubclusters, final_numSubclusters);
				}
...
int32_t merge_subclusters_in_bin(const cluster_opt_t *opt, cpu_cluster_record_t *clusterRecords, uint32_t niPET) {
	...
	if (numSubclusters>1) {
		...
		if (bwa_verbose>=5) {
			fprintf(stderr, "[D:merge_subclusters_in_bin] Before merge_subclusters\n");
			report_Subclusters_List (stderr, subclusters, numSubclusters, NULL);
		}
		...
		if (final_numSubclusters!=numSubclusters) {
			if (bwa_verbose>=5) {
				fprintf(stderr, "[D:merge_subclusters_in_bin] After merge_subclusters, #subclusters %d -> %d\n", numSubclusters, final_numSubclusters);
				report_Subclusters_List (stderr, subclusters, numSubclusters, NULL);
				
				fprintf(stderr, "[D:merge_subclusters_in_bin] Before adjustment\n");
			...
			if (bwa_verbose>=5) {
				fprintf(stderr, "[D:merge_subclusters_in_bin] merge_subclusters update order\n");
				report_Subclusters_List (stderr, subclusters, numSubclusters, NULL);
			}
			...
			if (bwa_verbose>=5) {
				fprintf(stderr, "[D:merge_subclusters_in_bin] After adjustment\n");
				report_iPETs_List (stderr, clusterRecords, niPET, NULL, -1);
			}
...
int32_t cluster_iPETs_Original(const cluster_opt_t *opt, cpu_cluster_record_t *clusterRecords, uint32_t nClusterKeys) {
	int32_t i,j;
	if (nClusterKeys>1) {
		for(i=0; i<nClusterKeys; ++i) {
			...
			if (clusterRecords[i].swap) {
				// if >=2 members, we will need to check if the overlap is proper
				int32_t niPET = clusterRecords[i].iPET;
				int32_t nRetryDiscordantPETs = cluster_bin(opt, &(clusterRecords[i]), niPET, 1, clusterRecords[i].pairId);
				...
				//if (bwa_verbose >= 5)
				{
					...
					if (niPET!=niPETinBin) {
						fprintf(stderr, "[D:iPET] i=%d, iPET=%d, iPETinBin=%d, #discordant=%d, #subclusters=%d\n", i, niPET, niPETinBin, nRetryDiscordantPETs, numSubclusters);
						fprintf(stderr, "[D:iPET] i=%d, iPETs=", i);
		...
		for(i=0; i<nClusterKeys; ++i) {
			...
			if (clusterRecords[i].swap) {
				...
				if (niPET>=opt->bulkiPETsTrigger) {
					...
					for(j=0; j<niPET; j+=opt->bulkiPETsBlock) {
						int32_t nChunkSize = niPET - j;
						if (nChunkSize>opt->bulkiPETsBlock) nChunkSize = opt->bulkiPETsBlock;

						if (clusterRecords[i+j].cid!=cid || clusterRecords[i+j].mcid!=mcid) {
							fprintf(stderr, "[D:%s] Different cluster group!\n", __func__);
						}
					...
					for(j=0; j<niPET; j+=opt->bulkiPETsBlock) {
						...
						if (bwa_verbose>=4) {
							fprintf(stderr, "[D:%s] Calling chunk (size=%d) clustering on %ld (%d iPETs)\n",
									__func__, nChunkSize, pairId, niPET);
						}
						
						int32_t nChunkRetryDiscordantPETs = cluster_bin(opt, &(clusterRecords[i+j]), nChunkSize, 1, pairId);
						nRetryDiscordantPETs += nChunkRetryDiscordantPETs;
						
						if (bwa_verbose>=4) {
							fprintf(stderr, "[D:%s] Calling chunk (size=%d) merging on %ld (%d iPETs) with %d discordant iPETs\n",
									__func__, nChunkSize, pairId, niPET, nChunkRetryDiscordantPETs);
						}
						
#if 0
						int32_t numChunkSubclusters = merge_subclusters_in_bin(opt, &(clusterRecords[i+j]), nChunkSize);
#else
						...
						if (bwa_verbose>=4) {
							fprintf(stderr, "[D:%s] Chunk (size=%d) processed on %ld (%d iPETs) with %d discordant iPETs and %d subclusters\n",
									__func__, nChunkSize, pairId, niPET, nChunkRetryDiscordantPETs, numChunkSubclusters);
						}
					...
					// DEBUG:
					if (129==niPET) {
						fprintf(stderr, "[D:merge_subclusters_in_bin] Chunk niPET==129 before final merger\n");
						report_iPETs_List (stderr, &(clusterRecords[i]), niPET, NULL, -1);
					}
					// DEBUG:END
#endif
					...
					if (129==niPET) {
						...
						fprintf(stderr, "[D:merge_subclusters_in_bin] Chunk niPET==129 after final merger\n");
						report_iPETs_List (stderr, &(clusterRecords[i]), niPET, NULL, -1);
					}
					// DEBUG:END
#endif
					...
					// DEBUG:
					if (129==niPET) {
						fprintf(stderr, "[D:merge_subclusters_in_bin] Chunk niPET==129 before cluster bin\n");
						report_iPETs_List (stderr, &(clusterRecords[i]), niPET, NULL, -1);
					}
					// DEBUG:END
#endif
					...
					// DEBUG:
					if (129==niPET) {
						fprintf(stderr, "[D:merge_subclusters_in_bin] Chunk niPET==129 before cluster bin merger\n");
						report_iPETs_List (stderr, &(clusterRecords[i]), niPET, NULL, -1);
					}
					// DEBUG:END
#endif
...
int main_cluster(int argc, char *argv[])
{
	...
	strcpy(in_mode, "r");
	while ((c = getopt(argc, argv, "mdsSt:O:l:u:b:5:3:m:i:v:B:")) >= 0) {
	...
	if (optind >= argc) {
		...
		fprintf(stderr, "       -m         use mid-point\n");
	...
	if (argc == optind + 1) { // convert/print the entire file
		...
		int32_t numUsables = nClusterKeys;
		for(i=0; i<nClusterKeys; ++i) {
		...
		qsort(clusterKeys, numUsables, sizeof(cpu_cluster_record_t), cmpPETTag1);
		t_timepointIO = realtime();
		t_diffProcessing = t_timepointIO - t_timepointProcessing;
		if (bwa_verbose >= 3) {
		...
		t_timepointProcessing = realtime();
		//cluster_PETags1(opt, clusterKeys, nClusterKeys);
		cluster_PETags1(opt, clusterKeys, numUsables);
		t_timepointIO = realtime();
		t_diffProcessing = t_timepointIO - t_timepointProcessing;
		if (bwa_verbose >= 3) {
		...
		// let's sort based on combine tag
		t_timepointProcessing = realtime();
		qsort(clusterKeys, numUsables, sizeof(cpu_cluster_record_t), cmpPETBothTags);
		t_timepointIO = realtime();
		t_diffProcessing = t_timepointIO - t_timepointProcessing;
		if (bwa_verbose >= 3) {
		...
		cluster_count_iPET(opt, clusterKeys, numUsables, &numClusterBins);
		t_timepointIO = realtime();
		t_diffProcessing = t_timepointIO - t_timepointProcessing;
		if (bwa_verbose >= 3) {
			fprintf(stderr, "[M::%s] Clustered %u combined key records, %u cluster bins, processing %.2f sec ( %.2f min)..\n", __func__, nClusterKeys, numClusterBins, t_diffProcessing, t_diffProcessing/60.0);
		}
		...
		report_Clusters_List (clustersfd, clusterKeys, numUsables, in->header);
